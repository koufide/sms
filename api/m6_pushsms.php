...
print "<br/><br/><br/>\n\n\n ========== EXECUTION MANUAL PUSH 6 SMS ====== <br/>\n" . $now->format('d/m/Y H:i:s');
...
$file_lock_push_sms = $param->files->file_lock_mpush_sms6;  ### 6 ieme diffusion
...
if ($dossier = opendir($push_rep)) {
    while (false !== ($fichier = readdir($dossier))) {
        if ($fichier != '.' && $fichier != '..' && $fichier != 'index.php' && $fichier != 'archive') {
            ...
            if (preg_match("/^M6PUSH/i", $filename)) {   ### 6 ieme diffusion


                $now = new DateTime('NOW', new DateTimeZone(('UTC')));
                print "<br/><br/><br/>\n\n\n ========== EXECUTION MANUAL 6 PUSH SMS ====== <br/>\n" . $now->format('d/m/Y H:i:s');
...
if ($trouve) {
    ...
    $myfile = file_put_contents('log/m6_pushsms_' . $now->format('Y_m_d') . '.log', $out1 . PHP_EOL, FILE_APPEND | LOCK_EX);
}
