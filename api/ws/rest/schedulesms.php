        ...
        if (!empty($rows)) {
            foreach ($rows as $key => $row) {
                ...
                if ($nbresms != '0' and $smsenvoye > $nbresms) {
                    ...
                    $fct->flooding($to, trim($text),  $now->format('d/m/Y'));
                    //exit("quiiter");
...
function retour_json($user, $retourErreur, $retourMessage, $retour)
{
    ...
    if ($user == null)
        $myfile = file_put_contents('log/schedulesms_' . $now2->format('Y_m_d') . '.log', $out1 . PHP_EOL, FILE_APPEND | LOCK_EX);
    else {
        $myfile = file_put_contents('log/' . $user . '_schedulesms__' . $now2->format('Y_m_d') . '.log', $out1 . PHP_EOL, FILE_APPEND | LOCK_EX);
    }
