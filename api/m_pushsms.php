...
print "<br/><br/><br/>\n\n\n ========== EXECUTION MANUAL PUSH 1 SMS ====== <br/>\n" . $now->format('d/m/Y H:i:s');
...
$file_lock_push_sms = $param->files->file_lock_mpush_sms;  ### 1 ieme diffusion
    ...
    while (false !== ($fichier = readdir($dossier))) {
        if ($fichier != '.' && $fichier != '..' && $fichier != 'index.php' && $fichier != 'archive') {
            ...
            if (preg_match("/^MPUSH/i", $filename)) {
                ...
                $now = new DateTime('NOW', new DateTimeZone(('UTC')));
                print "<br/><br/><br/>\n\n\n ========== EXECUTION MANUAL 1ere DIFF PUSH SMS ====== <br/>\n" . $now->format('d/m/Y H:i:s');
...
if ($trouve) {
    ...
    $myfile = file_put_contents('log/m_pushsms_' . $now->format('Y_m_d') . '.log', $out1 . PHP_EOL, FILE_APPEND | LOCK_EX);
}
