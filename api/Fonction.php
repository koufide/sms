...
class Fonction
{
    ...
    public function __construct()
    {
        ...
        $this->delai_sms = 20; # delai d attente avant d envoyer le sms/ juste pour les INCIDENTS MONETIQUES
    ...
    /**
     * get_delai_sms : retourne le delai d attente en minutes
     */
    ...
    /**
     * $from : emetteur du sms
     * $to : destinataire
     * $text: le message envoyé
     * 
    ...
    public function interceptSMS($from, $to, $text)
    {
        ...
        if ($from == 'INCIDENT MO') {
            ...
            TIMEDIFF(  now(), str_to_date(o.sendsms_at,'%Y-%m-%d %H:%i:%s')  ) diff_date,
            EXTRACT(MINUTE FROM TIMEDIFF(now(),  str_to_date(o.sendsms_at,'%Y-%m-%d %H:%i:%s') )  )  extract_min,
             o.* 
            from outgoing o 
            where  o.a = :tel AND o.de = :de
            and substr(o.text,22) = :text and date_format(o.sendsms_at,'%Y-%m-%d') = date_format(now(),'%Y-%m-%d') 
            ...
            EXTRACT(MINUTE FROM TIMEDIFF(now(),  str_to_date(o.sendsms_at,'%Y-%m-%d %H:%i:%s') )  )  extract_min,
             o.* 
            from outgoing o 
            where  o.a = :a AND o.de = :de 
            and date_format(o.sendsms_at,'%Y-%m-%d') = date_format(now(),'%Y-%m-%d') 
            order by id desc limit 0,1";
            ...
            // var_dump($outgoings);
            // var_dump($outgoings[0]['extract_min']);
            ...
            if (empty($outgoings)) {
                ...
                if ($extract_min >= $delai_ref) {
                    ...
                    // $retourErreur = '0';
                    // $retourMessage = "from: $from to: $to sms: $text ,,,interceptSMS,,, $new_text # $sms ,,,";
                    // $user='test';
                    // $retour=[];
    ...
    /**
     * cette fonction n interrompt pas la suite des triatements
     * juste une notification/log
     */
    // function retour_json_mo($user, $retourErreur, $retourMessage, $retour)
    function retour_json_mo($user, $retourErreur, $retourMessage, $retour)
    {
        ...
        if ($user == null)
            $myfile = file_put_contents('/var/www/html/sms/api/ws/rest/log/sendsms_' . $now2->format('Y_m_d') . '.log', $out1 . PHP_EOL, FILE_APPEND | LOCK_EX);
        else {
